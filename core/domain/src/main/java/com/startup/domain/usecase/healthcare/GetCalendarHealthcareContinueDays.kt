...
class GetCalendarHealthcareContinueDays @Inject constructor(private val healthcareRepository: HealthcareRepository) :
    BaseUseCase<Int, Unit>() {
    override fun invoke(params: Unit): Flow<Int> =
        healthcareRepository.getCalendarHealthcareContinueDays()
}