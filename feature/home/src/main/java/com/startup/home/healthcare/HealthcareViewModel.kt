...
class HealthcareViewModel @Inject constructor(
    ...
    private val _state = HealthcareViewStateImpl(
        currentContinueDays = getCalendarHealthcareContinueDays.invoke(Unit).stateInViewModel(0)
    )
    ...
    override fun handleViewModelEvent(event: BaseEvent) {
        when (event) {
            ...
            is HealthcareUiEvent.OnGoalChanged -> {
                // TODO PUT
            }
        }
    }
    ...
    private fun fetchCurrentDateDetail(date: LocalDate) {
        getDailyHealthcareDetail.invoke(DateUtil.convertTranslatorDateFormat(date))
            .onStart {
                BaseUiState(
                    isShowShimmer = true,
